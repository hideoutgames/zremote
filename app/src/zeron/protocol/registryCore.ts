// Ported from zeron@853872d edge/src/registry-core.ts (1:1) and
// apps/ios/Zeron/Sync/RegistryCore.swift (HlcClock).
//
// Registry merge core — the pure row/op semantics behind the registry room
// (docs/registry-sync.md). The registry stores CURRENT STATE ONLY: a row is
// a bag of fields, each field carries the HLC of its last write, and a write
// applies iff its clock beats the stored one.
...
export const encodeHlc = (ms: number, counter: number, device: string): Hlc =>
  `${String(ms).padStart(13, '0')}-${String(counter).padStart(
    6,
    '0',
  )}-${device}`;

/** `a` strictly newer than `b` (undefined = never written, loses to any). */
export const hlcNewer = (a: Hlc, b: Hlc | undefined): boolean =>
  b === undefined || a > b;

/** Monotonic HLC source (RegistryCore.swift HlcClock): never emits the same
 * or an earlier clock twice, even across a wall-clock regression or restart
 * (persist `lastMs`/`counter` with the doc state). */
export class HlcClock {
  lastMs = 0;
  counter = 0;

  next(nowMs: number, device: string): Hlc {
    if (nowMs > this.lastMs) {
      this.lastMs = nowMs;
      this.counter = 0;
    } else {
      this.counter += 1;
      if (this.counter > 999_999) {
        this.lastMs += 1;
        this.counter = 0;
      }
    }
    return encodeHlc(this.lastMs, this.counter, device);
  }
}
...
export const ID_RE = /^[A-Za-z0-9_.:@/-]{1,256}$/;
export const KIND_RE = /^[a-z][a-zA-Z0-9]{0,31}$/;
export const FIELD_RE = /^[a-zA-Z][a-zA-Z0-9]{0,63}$/;
export const HLC_RE = /^\d{13}-\d{6}-[A-Za-z0-9_-]{1,128}$/;
...
export const validateOp = (op: Op): string | null => {
  ...
  if (op.op !== 'upsert' && op.op !== 'update' && op.op !== 'delete')
    return 'bad op';
...
export const applyOp = (row: Row | undefined, op: Op): ApplyResult => {
  ...
  if (op.op === 'delete') {
    ...
    if (
      row.deleted
        ? hlcNewer(op.hlc, row.delHlc)
        : hlcNewer(op.hlc, maxClock(row))
    ) {
      return {
...
export const rowToSeedOp = (row: Row): Op => {
  if (row.deleted) {
    return {
      ...
      id: row.id,
      op: 'delete',
      hlc: row.delHlc ?? encodeHlc(0, 0, 'seed'),
    };
  }
