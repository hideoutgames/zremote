...
- **PRs always target `main`.** Feature branches and worktrees are for
  ...
  Cursor branch. TestFlight is **manual dispatch only** (typically of
  `main`); stacked PRs never ship. Rebase onto `main` and retarget the
  PR if the work started on a stacked branch.
