...
| Queued attachments (committed host paths on the queue row; upload before the row is written)                     | capability `message-queue-attachments-v1` | `doc_host.rs` `queued_message_prompt` copies `attachments` into the prompt and does not rewrite `pending://`. `sendWithAttachments` uploads first; `sendPlan()` falls back to legacy upload-first without the capability.                                                                                                                                                             |
