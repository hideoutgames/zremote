...
| Native services                | `zeron/native/` and `app/modules/*`                | `react-native-loro` (Nitro over loro-swift), `zeron-dictation` (SpeechAnalyzer / on-device SFSpeechRecognizer), local Voice Model ports (`voiceCapture` / `transcription` / `cleanup` — Mac-gated whisper.rn/llama.rn adapters), Live Activities (`expo-widgets`), secure storage, haptics. Each has a JS port interface so Jest runs without them.                                                                                                                                          |
